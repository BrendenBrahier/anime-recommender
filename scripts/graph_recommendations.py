...
def build_graph(data, similarity_matrix, threshold=0.2):  # Lowered threshold further
    graph = nx.Graph()
    
    # Add nodes with just names
    for i, anime in data.iterrows():
        graph.add_node(i, name=anime['name_english'])

    # Add edges using pre-calculated similarity
...
def get_recommendations(graph, anime_title, data, indices, top_n=10):
    ...
    # Limit the number of neighbors printed
    print(f"Neighbors for {anime_title}: {neighbors[:10]} (total: {len(neighbors)})")
    ...
    # If fewer than top_n neighbors, include additional neighbors
    ...
    for i, _ in top_scores:
        ...
        if anime_name != anime_title:  # Exclude the input anime
            recommended_anime.append(anime_name)
    
    print(f"Graph recommendations for {anime_title}: {recommended_anime}")
